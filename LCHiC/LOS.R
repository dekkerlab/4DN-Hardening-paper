...
parser$add_argument("-i", help="Restriction enzyme digest sample cispercent.bedGraph", type="character",
                    required=TRUE)
parser$add_argument("-m", help="Mock nuclei control cispercent.bedGraph", type="character", required=TRUE)
args <- parser$parse_args()

M_cis_df <- read.table(args$m, sep="\t", header=TRUE)
D_cis_df <- read.table(args$i, sep="\t", header=TRUE)
# Fix bp indexing
